...
type ROSPackage
    ...
    msg::ROSModule
    srv::ROSModule
    function ROSPackage(pkgname::AbstractString)
        pkg = new(pkgname)
...
type ROSMsgModule <: ROSModule
    ...
    deps::Set{AbstractString}
    ROSMsgModule(pkg) = new(pkg, AbstractString[], Set{AbstractString}())
end
type ROSSrvModule <: ROSModule
    pkg::ROSPackage
    members::Vector{AbstractString}
    deps::Set{AbstractString}
    ROSSrvModule(pkg) = new(pkg, AbstractString[], Set{AbstractString}())
end
...
const _rospy_imports = Dict{AbstractString,ROSPackage}()
const _rospy_objects = Dict{AbstractString,PyObject}()
const _rospy_modules = Dict{AbstractString,PyObject}()

const _ros_builtin_types = Dict{AbstractString, Symbol}(
    "bool"    => :Bool,
    "int8"    => :Int8,
    "int16"   => :Int16,
    "int32"   => :Int32,
...
function _rosimport(package::AbstractString, ismsg::Bool, names::AbstractString...)
...
function addtype!(mod::ROSMsgModule, typ::AbstractString)
    global _rospy_objects
...
#first.
function addtype!(mod::ROSSrvModule, typ::AbstractString)
    global _rospy_objects
    if !(typ in mod.members)
...
function _pyvars(modname::AbstractString, typ::AbstractString)
...
function _import_rospy_pkg(package::AbstractString)
...
function buildtype(mod::ROSMsgModule, typename::AbstractString)
    global _rospy_objects
...
function buildtype(mod::ROSSrvModule, typename::AbstractString)
    global _rospy_objects
...
function typecode(rosname::AbstractString, super::Symbol, members::Vector)
    ...
    push!(exprs, :(
        function convert(jlt::Type{$jlsym}, o::PyObject)
            if convert(AbstractString, o["_type"]) != _typerepr(jlt)
                throw(InexactError())
            end
            jl = $jlsym()
            #Generated code here
...
_rostypestr(mod::ROSModule, name::AbstractString) = string(_name(mod),"/",name)
function _splittypestr(typestr::AbstractString)
    if ! _isrostype(typestr)
...
_isrostype(s::AbstractString) = ismatch(r"^\w+/\w+(?:\[\d*\])?$", s)
...
function _check_array_type(typ::AbstractString)
    arraylen = -1
    m = match(r"^([\w/]+)\[(\d*)\]$", typ)
    if m != nothing
        btype = m.captures[1]
...
_nameconflicts(typename::AbstractString) = isdefined(Base, Symbol(typename))

#Get a default value for any builtin ROS type
_typedefault{T<:Real}(::Type{T}) = zero(T)
_typedefault(::Type{AbstractString}) = ""
